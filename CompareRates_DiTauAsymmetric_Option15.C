{
  ...
  TH2F* rate_NewLayer1_Option15_mean = (TH2F*)f_mean.Get("rate_DiTau_Progression_15");

  TString CanvasName = "./PlotIsolation/PlotRate/Comparison_Rate_DiTau_Asymmetric_Run305310_92X_mean_Option15";
  ...
  tex2->AddText("#splitline{Linearly scaled to 2.0E34}{Iso Option 15}");
  ...
  TPaveText* tex9kHz = new TPaveText(0.13,0.50,0.2,0.60,"NDC");
  ...
  TPaveText* tex14kHz = new TPaveText(0.13,0.31,0.2,0.41,"NDC");
  ...
  TMarker* sym9 = new TMarker(43,43,20);
  sym9->SetMarkerSize(1.5);
  sym9->SetMarkerColor(8);
  sym9->Draw("same");

  TMarker* sym9a = new TMarker(46,43,22);
  sym9a->SetMarkerSize(1.7);
  sym9a->SetMarkerColor(8);
  //sym9a->Draw("same");

  TMarker* sym9b = new TMarker(48,42,21);
  sym9b->SetMarkerSize(1.5);
  sym9b->SetMarkerColor(8);
  sym9b->Draw("same");

  TMarker* sym9c = new TMarker(46,28,23);
  sym9c->SetMarkerSize(1.7);
  sym9c->SetMarkerColor(6);
  //sym9c->Draw("same");

  TMarker* sym9d = new TMarker(48,27,33);
  sym9d->SetMarkerSize(2);
  sym9d->SetMarkerColor(6);
  //sym9d->Draw("same");
  
  TMarker* sym14 = new TMarker(34,34,20);
  ...
  TLegend* leg2 = new TLegend(0.57,0.78,0.82,0.85);
  leg2->SetBorderSize(0);
  leg2->SetTextSize(0.025);
  leg2->SetHeader("#bf{Asymmetric di-tau, 9 kHz}");
  //leg2->AddEntry(sym9a,"(46,43) GeV","p");
  leg2->AddEntry(sym9b,"(48,42) GeV","p");
  //leg2->AddEntry(sym9c,"(46,28) GeV","p");
  //leg2->AddEntry(sym9d,"(48,27) GeV","p");
  ...
  c.SaveAs("PlotIsolation/PlotRate/Rate_tau_asym_305310_Option15_9kHz.pdf","pdf");
  ...
  for(UInt_t j = 1 ; j <= rate_NewLayer1_Option15_mean->GetNbinsY() ; ++j)
  {
    for(UInt_t i = 1 ; i <= rate_NewLayer1_Option15_mean->GetNbinsX() ; ++i)
    {
      if(rate_NewLayer1_Option15_mean->GetBinContent(i,j)<=Target){
        if(rate_NewLayer1_Option15_mean->GetXaxis()->GetBinLowEdge(i) == 48){
