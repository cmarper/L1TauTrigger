{
  ...
  TH2F* rate_NewLayer1_Option23_mean = (TH2F*)f_mean.Get("rate_DiTau_Progression_23");

  TString CanvasName = "../PlotIsolation/PlotRate/Comparison_Rate_DiTau_Asymmetric_Run305310_92X_mean_Option23";
  ...
  tex2->AddText("#splitline{Linearly scaled to 2.0E34}{Iso Option 23}");
  tex2->SetTextSize(0.025);
  tex2->SetFillStyle(0);
  tex2->SetBorderSize(0);

  TPaveText* tex7kHz = new TPaveText(0.13,0.51,0.2,0.61,"NDC");
  ...
  TPaveText* tex9kHz = new TPaveText(0.13,0.30,0.2,0.40,"NDC");
  ...
  TPaveText* tex14kHz = new TPaveText(0.13,0.16,0.2,0.26,"NDC");
  ...
  gStyle->SetPalette(1,colors_23); 
  TH2F *hcontours_Option23 = (TH2F*) rate_NewLayer1_Option23_mean->Clone();
  hcontours_Option23->SetFillColor(0);
  hcontours_Option23->SetContour(4,contours);
  gStyle->SetPalette(4,colors_23);
  hcontours_Option23->SetLineWidth(2);
  hcontours_Option23->Draw("cont1 list");//cont1 for line plot!
  hcontours_Option23->SetTitle("");
  hcontours_Option23->GetXaxis()->SetRangeUser(20.,60.);
  hcontours_Option23->GetYaxis()->SetRangeUser(20.,60.);
  hcontours_Option23->GetXaxis()->SetTitle("Leading #tau E_{T} threshold [GeV]");
  hcontours_Option23->GetYaxis()->SetTitle("Sub-leading #tau E_{T} threshold [GeV]");
  gPad->Update();
  ...
  TMarker* sym7 = new TMarker(42,42,20);
  sym7->SetMarkerSize(1.5);
  sym7->SetMarkerColor(2);
  sym7->Draw("same");
  
  TMarker* sym9 = new TMarker(31,31,20);
  sym9->SetMarkerSize(1.5);
  sym9->SetMarkerColor(8);
  sym9->Draw("same");

  TMarker* sym9a = new TMarker(40,29,22);
  ...
  TMarker* sym9c = new TMarker(44,26,23);
  sym9c->SetMarkerSize(1.7);
  sym9c->SetMarkerColor(8);
  sym9c->Draw("same");

  TMarker* sym9d = new TMarker(46,25,33);
  ...
  TMarker* sym9g = new TMarker(52,21,21);
  sym9g->SetMarkerSize(1.5);
  sym9g->SetMarkerColor(8);
  sym9g->Draw("same");
  
  TMarker* sym14 = new TMarker(24,24,20);
  ...
  leg->AddEntry(sym7,"7 kHz: (42,42) GeV","p");
  leg->AddEntry(sym9,"9 kHz: (31,31) GeV","p");
  leg->AddEntry(sym14,"14 kHz: (24,24) GeV","p");
  //leg->AddEntry(sym25,"25 kHz: (27,27) GeV","p");
  
  TLegend* leg2 = new TLegend(0.57,0.62,0.82,0.88);
  leg2->SetBorderSize(0);
  leg2->SetTextSize(0.025);
  leg2->SetHeader("#bf{Asymmetric di-tau, 9 kHz}");
  leg2->AddEntry(sym9a,"(40,29) GeV","p");
  leg2->AddEntry(sym9b,"(42,28) GeV","p");
  leg2->AddEntry(sym9c,"(44,26) GeV","p");
  leg2->AddEntry(sym9d,"(46,25) GeV","p");
  leg2->AddEntry(sym9e,"(48,24) GeV","p");
  leg2->AddEntry(sym9f,"(50,23) GeV","p");
  leg2->AddEntry(sym9g,"(52,21) GeV","p");
  ...
  c.SaveAs("../PlotIsolation/PlotRate/Rate_tau_asym_305310_Option23_9kHz.pdf","pdf");
...
*/ 
  ...
  for(UInt_t j = 1 ; j <= rate_NewLayer1_Option23_mean->GetNbinsY() ; ++j)
  {
    for(UInt_t i = 1 ; i <= rate_NewLayer1_Option23_mean->GetNbinsX() ; ++i)
    {
      if(rate_NewLayer1_Option23_mean->GetBinContent(i,j)<=Target){
       // if(rate_NewLayer1_Option23_mean->GetXaxis()->GetBinLowEdge(i) == 52){
        if(rate_NewLayer1_Option23_mean->GetXaxis()->GetBinLowEdge(i) == rate_NewLayer1_Option23_mean->GetYaxis()->GetBinLowEdge(j)){
           cout << "Thr 1 " << rate_NewLayer1_Option23_mean->GetXaxis()->GetBinLowEdge(i) << " " ;
  	   cout << "Thr 2 " << rate_NewLayer1_Option23_mean->GetYaxis()->GetBinLowEdge(j) << endl;
           break;
           }
       //break;
       }  
